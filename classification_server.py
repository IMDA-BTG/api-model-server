...
# image_model = load_model("model/bc_image_face")

def predict_result(df):
  prediction = model.predict(df)
  return int(prediction[0])

def predict_result_proba(df):
...
def get_array_json_proba(request):
  payload = request.get_json()
  results = []
  for row in payload:
    df = get_single_row(row)
    result = predict_result_proba(df)
...
# #   # print("request_json", request_json)
# #   # return None
# #   result = predict_proba_batched_result(request_json)
# #   print("predict_proba_batched_result", result)
...
#   result = predict_proba_batched_result(request_json)
#   print("predict_proba_batched_result", result)
#   dumped_json_result = json.dumps(result.tolist())
#   response = make_response(jsonify({'data': dumped_json_result}), 200)
#   return log_and_return_response(response)
...
def predict_tc013():  
  ...
  # results = get_array_json_proba(request)
